...
namespace Ultra.Inventory {
    public partial class LocView :  BaseForm , Ultra.Surface.Interfaces.ISurfacePermission {
        ...
        void btnUnUsing_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e) {
            ...
            if (et == null)
                return;
            using (var db = new Database()) {
                db.Execute("update t_location set isusing=0 where guid = @0 ",et.Guid);
                btnRefresh_ItemClick(null, null);
            }
        }
        ...
        void btnRefresh_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e) {
            using (var db = new Database()) {
                gc.DataSource = db.Fetch<t_location>(string.Empty);
            }
        }
    }
}
